...
class Peer:
    def __init__(self, peer_id, server_host, server_port):  # 接受用户输入的 server_host 和 server_port
    ...
    def add_resource(self, file_path):
        ...
        try:
            ...
            print(fnf_error)
            return f"Failed to add resource: {str(fnf_error)}"
        except Exception as e:
            return f"Failed to add resource: {str(e)}"

def check_peer_id_unique(peer_id, server_host, server_port):
    # 检查对等节点 ID 是否唯一
    request = {
        'action': 'retrieve_peers'
    }
...
def main():
    # 提示用户输入central server的地址和端口号
    server_host = input("Enter the central server host: ")
    server_port = int(input("Enter the central server port: "))
    ...
    while True:
        ...
        if check_peer_id_unique(peer_id, server_host, server_port):
            break
    # 初始化peer
    peer = Peer(peer_id, server_host, server_port)
